...
/*   Updated: 2023/06/22 13:33:51 by avaganay         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    move_nwes(t_all *all, char c)
{
	...
	if (c == 'W')
	{
		...
		{
			all->ray->pos.x -= all->ray->dir.y * all->ray->moveSpeed;
			all->ray->pos.y += all->ray->dir.x * all->ray->moveSpeed;
		}
	}
	if (c == 'E')
	{
	...
	if (c == 'S')
	{
		...
		{
			all->ray->pos.y -= all->ray->dir.y * all->ray->moveSpeed;
			all->ray->pos.x -= all->ray->dir.x * all->ray->moveSpeed;
		}
	}
	if (c == 'N')
	{
