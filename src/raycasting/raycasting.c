...
{
	all->ray.camera_orientation = 2 * i / (double)1920 - 1;
...
void	draw_wall(t_all *all, int i)
{
	...
	all->ray.draw_start = -all->ray.line_height / 2 + 1080 / 2;
	if (all->ray.draw_start < 0)
		all->ray.draw_start = 0;
	all->ray.draw_end = all->ray.line_height / 2 + 1080 / 2;
	if (all->ray.draw_end >= 1080)
